            ...
            <div key = {dog.name + "-1"}>
                <div key = {dog.name} onClick = {dogSelectACB} className="dogResults border-solid border-4 border-violet-100 my-10 rounded-lg bg-gradient-to-tl from-violet-100 to-orange-400 bg-opacity-50 font-mono font-extrabold text-xl">
                    ...
                    <a href="#/result-details">
                        <button key = {dog.name + "2"} className=' font-bold  bg-green-300 mb-5 p-5 rounded-full mt-4 mr-2 min-w-max'>{dog.name}</button>
                    </a>
                    </div>
                    <button key = {dog.name + "3"} onClick={dogRemoveACB} className="font-bold bg-red-400 mb-5 p-5 rounded-full w-12 h-12">x</button>
                </div>
            </div>
        );
    }
